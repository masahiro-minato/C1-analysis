...
# データ長 #1188
length(C1.MIF_by.date$X.date)
C1.MIF_by.date$X.date[93]
...
# データ
data_list <- list(
  y = C1.MIF_by.date$EM.count[start_day:end_day],
  Num = C1.MIF_by.date$MIF.cumsum[start_day:end_day]/max(C1.MIF_by.date$MIF.cumsum[start_day:end_day]), # 正規化が必要
  T = end_day-start_day+1,
  N_max = max(C1.MIF_by.date$MIF.cumsum[start_day:end_day]),
  grainsize = 250
)
# サンプリング開始日/終了日
C1.MIF_by.date$X.date[start_day]
C1.MIF_by.date$X.date[end_day]
...
fit <- mod.sample(
  ...
  max_treedepth = 15,
  refresh = 500,
  output_basename = paste0(Phenom,"-",format(Sys.time(), "%H:%M:%S")),
  output_dir = paste0("./csv/",Phenom),
  object.path = str_c("./Cmdstan_files/Metis-C1.system.EM.fit.",Phenom,".rds"),
  graph.path = str_c("./PDF/Metis-C1-EM_AR.tvc.period_poisson.",Phenom,".pdf")
)
...
fit <- read_rds(str_c("./Cmdstan_files/Metis-C1.system.EM.fit.",Phenom,".rds"))
fit$print(c("s_w", "s_s", "s_r", "s_t", "b_ar", "b_ope[1]", "Intercept", "lp__"))
fit %>% bayesplot::rhat() %>% hist
...
date_plot <- 
  seq(
    from = as.POSIXct(C1.MIF_by.date$X.date[start_day]),
    by = "days",
    len = end_day-start_day+1
  )
...
p_lambda_pois <- plotSSM.CmdStanr(fit = fit, 
                         time_vec = date_plot,
                         obs_vec = C1.MIF_by.date$EM.count[start_day:end_day],
                         state_name = "lambda_exp", 
                         graph_title = "λ(μ + γ + tvc + r)：すべての成分を含んだ状態推定値", 
                         y_label = "件数",
                         date_labels = "%Y/%m",
...
# グラフ保存
ggsave(str_c("./PDF/Metis-C1-EM_AR.tvc.period_poisson.",Phenom,".pdf"), 
